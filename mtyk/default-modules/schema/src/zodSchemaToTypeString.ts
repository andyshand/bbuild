...
export default function zodSchemaToTypeString(
  schema: z.ZodTypeAny,
  typeName: string
): string {
  const parents: any[] = []
  function processZodType(zodType: z.ZodTypeAny): string {
    if (!zodType) {
      return 'any'
    }
    ...
    if (typeName === 'ZodString') {
      ...
      return `${processZodType(
        (zodType as z.ZodArray<any>)._def.type as any
      )}[]`
    } else if (typeName === 'ZodEnum') {
      const enumValues = (zodType as z.ZodEnum<any>)._def.values.map(
        (value) => `"${value}"`
      )
      ...
      const fieldStrings = Object.entries(
        (zodType as z.ZodObject<any>).shape
      ).map(([fieldName, fieldSchema]: any) => {
        let optional = fieldSchema._def.isOptional ? '?' : ''
        let fieldType = processZodType(fieldSchema as any)
      ...
      const valueType = processZodType(
        (zodType as z.ZodMap<any>)._def.valueType
      )
      return `Map<${keyType}, ${valueType}>`
    } else if (typeName === 'ZodRecord') {
      const valueType = processZodType(
        (zodType as z.ZodRecord<any>)._def.valueType
      )
      return `Record<string, ${valueType}>`
    } else if (typeName === 'ZodUnion') {
      const unionTypes = (zodType as z.ZodUnion<any>)._def.options.map(
        (option) => processZodType(option)
      )
      return unionTypes.join(' | ')
    } else if (typeName === 'ZodIntersection') {
      const leftType = processZodType(
        (zodType as z.ZodIntersection<any, any>)._def.left
      )
      const rightType = processZodType(
        (zodType as z.ZodIntersection<any, any>)._def.right
      )
      ...
      const optionalType = processZodType(
      ...
      const effectsType = processZodType(
        (zodType as z.ZodEffects<any>)._def.schema
      )
      return effectsType
    } else {
