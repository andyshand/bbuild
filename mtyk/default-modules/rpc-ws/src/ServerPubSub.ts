...
export default class ServerPubSub {
  ...
  constructor(public readonly server: WSServer) {
    ...
    this.server.functions['getChannelObservable'] = (channel: string) =>
      this.getChannelObservable(channel)
    this.server.functions['publish'] = (data: { channel: string; payload: any }) =>
      this.publish(data.channel, data.payload)
    this.server.functions['setClientValue'] = (data: { channel: string; value: any }) =>
      this.setClientValue(data.channel, data.value)
    this.server.functions['clearClientValue'] = (channel: string) =>
      this.clearClientValue(channel)
  ...
  clearClientValue(
    channel: string,
    connectionId: string = getAsyncContext().get('connectionId')
  ) {
