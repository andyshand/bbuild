...
const TextareaComponent = observer(
  ({
    ...
    const componentEditWf = null as any // TODO removed since API changed
    const inputNodes =
      componentEditWf?.nodes.filter((n) => n.type === 'input') ?? []
    ...
    return (
      <div>
        ...
        {selectedComponents.length > 1 && (
          <div className="flex flex-row items-center flex-wrap gap-2">
            {selectedComponents.map((id) => (
              <div
                key={id}
                className="px-2 py-0.5 bg-gray-400 text-white rounded-full text-xs dark:bg-gray-700 font-medium"
                title={id}
              >
                {id.split('/').slice(-1)[0]}
              </div>
        ...
        <MTYKSelect
          options={inputNodes?.map((n) => (n?.data as any).name ?? n.id)}
          singleValue={selectedInputId}
      ...
      return (
        <HoverableComponent key={id} boundingBox={boundingBox} name={fileName}>
          <a
            ...
            className={`block hover:bg-gray-200 dark:hover:bg-gray-700 p-1 rounded ${
