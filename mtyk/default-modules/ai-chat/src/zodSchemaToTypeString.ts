...
export default function zodSchemaToTypeString(schema: z.ZodTypeAny, typeName: string): string {
	...
	function processZodType(zodType: z.ZodTypeAny): string {
		...
		if (typeName === "ZodString") {
			...
			const tupleTypes = (zodType as z.ZodTuple<any>)._def.items.map((item) => processZodType(item));
			return `[${tupleTypes.join(", ")}]`;
		} else if (typeName === "ZodObject") {
			const fieldStrings = Object.entries((zodType as z.ZodObject<any>).shape).map(([fieldName, fieldSchema]: any) => {
			...
			const unionTypes = (zodType as z.ZodUnion<any>)._def.options.map((option) => processZodType(option));
